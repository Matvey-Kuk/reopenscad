// ==========================================================
//  Tablet / pill box  —  INTERNAL volume 95 x 20 x 20 mm
//  Same design as tablet-box-75x50x25, re-driven from the
//  interior. Only iL/iW/iH and kn_n differ.
//
//  Print-in-place, both halves flat, folded closed.
//
//  Hinge: 17 narrow knuckles (3.73 mm each) across the long
//  back edge. No pins: each lid barrel ends in a conical
//  boss in a conical socket, slope 1.15, so boss, socket
//  wall and socket ceiling all stay under 45 deg. A 45 deg
//  buttress carries the barrel down into the wall.
//
...
/* ---------------- hinge ------------------- */
kn_n      = 17;     // more of them, so each stays ~3.7 mm wide
kn_gap    = 0.35;
cone_s    = 1.15;
cone_eng  = 0.80;
cone_tip  = 0.35;
...
hook_tip  = rec - hook_out;
travel    = wall - hook_tip;
strip     = win_z0 - skirt_z0;
...
// the shallow box makes this one worth checking explicitly:
assert(btn_z0 + btn_h + slot < split - 1.0, "latch runs into the rim");
assert(D - 2 * corner > 4, "box too shallow for the corner chamfer");
